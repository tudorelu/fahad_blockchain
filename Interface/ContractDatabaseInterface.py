...
TODO:
	...
	Test Device functionality (create device, assign device to user, pass ownership of device).

	Create Access Rights Control. 
	(will, at first, work by encrypting all the data with the private key of the administrator)
	(then, there will be mappings saved on blockchain that will contain addresses of entities 
	and the entities who have read / write access to the initial entities)

	Define Simulation variables & plan.
...
class ContractDatabaseInterface:
	...
	def __init__(self):
		...
		self.system_admin			= self.w3.eth.accounts[0]

		# self.contract_instance 		= self.create_new_contract_instance()
		self.contract_instance 		= self.get_contract_instance_from_address('0xc33eadeE43e8e3Cc7bb152113cC4C13F7C66f6bA');
	...
	def contract_transact(self, function_name:str, args:tuple=(), acct_num:int=0):
		""" Call contract function
		returns: True if function call was succesfull, False otherwise """
		tx_hash = ''

		if len(args) > 1:
			tx_hash = self.contract_instance.functions[function_name](*args).transact({'from':self.w3.eth.accounts[acct_num]})
		else :
			tx_hash = self.contract_instance.functions[function_name](args).transact({'from':self.w3.eth.accounts[acct_num]})
	...
	def contract_call(self, function_name:str, args:tuple=()):
		...
		print("args is")
		print(args)
		print("len of args is")
		print(len(args))

		if len(args) > 1:
			self.contract_instance.functions[function_name](*args).call()
		else:
		'''
